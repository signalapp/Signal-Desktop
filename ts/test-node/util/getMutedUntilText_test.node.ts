...
describe('getMutedUntilText', () => {
  ...
  it('returns an "always" label if passed a large number', () => {
    assert.strictEqual(
      getMutedUntilText(MuteExpiration.ALWAYS, i18n),
      'Muted always'
    );
    assert.strictEqual(
      getMutedUntilText(MuteExpiration.fromNumber(Infinity), i18n),
      'Muted always'
    );
  });
  ...
  it('returns the time if the mute expires later today', () => {
    assert.strictEqual(
      getMutedUntilText(
        MuteExpiration.fromNumber(new Date(2000, 3, 20, 18, 30, 0).valueOf()),
        i18n
      ),
      'Muted until 6:30 PM'
    );
  ...
  it('returns the date and time if the mute expires on another day', () => {
    assert.strictEqual(
      getMutedUntilText(
        MuteExpiration.fromNumber(new Date(2000, 3, 21, 18, 30, 0).valueOf()),
        i18n
      ),
