...
type AliceBasisStore = Record<string, Record<string, string>>;
// serviceId -> deviceId -> nonce_b64

const KEY = 'pvrfDemoAliceBasis' as const;

async function readStore(key?: any): Promise<AliceBasisStore> {
  const realKey =  (key || KEY)
  return (await itemStorage.get(realKey)) ?? {};
}

async function writeStore(store: AliceBasisStore, key?: any): Promise<void> {
  await itemStorage.put(key || KEY, store);
}

export async function setAliceBasis(
  serviceId: string,
  deviceId: number,
  nonceB64: string,
  key?: string
...
export async function getAliceBasis(
  serviceId: string,
  deviceId: number,
  key?: string
...
export async function clearAliceBasis(
  serviceId: string,
  deviceId: number
): Promise<void> {
