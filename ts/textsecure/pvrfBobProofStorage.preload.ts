...
type BobProofStore = Record<string, Record<string, string>>;
// serviceId -> deviceId -> nonce_b64

const KEY = 'pvrfDemoBobProof' as const;

async function readStore(key?: any): Promise<BobProofStore> {
  const realKey =  (key || KEY)
  return (await itemStorage.get(realKey)) ?? {};
}

async function writeStore(store: BobProofStore, key?: any): Promise<void> {
  await itemStorage.put(key || KEY, store);
}

export async function setBobProof(
  serviceId: string,
  deviceId: number,
  nonceB64: string,
  key?: string
...
export async function getBobProof(
  serviceId: string,
  deviceId: number,
  key?: string
...
export async function clearBobProof(
  serviceId: string,
  deviceId: number
): Promise<void> {
